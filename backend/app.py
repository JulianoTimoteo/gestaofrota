...
def _cpu_monitor_loop():
    """Thread daemon que mede CPU a cada 3s sem bloquear requests."""
    global _cpu_percent
    import time as _time
    while True:
        try:
            _cpu_percent = psutil.cpu_percent(interval=2)
        except Exception:
            pass
        _time.sleep(1)
...
def require_admin(f):
    ...
    def decorated(*args, **kwargs):
        ...
        sessao = conn.execute('''
            ...
            FROM sessoes s 
            JOIN usuarios u ON s.usuario_id = u.id 
            WHERE s.token = ? AND s.ativo = 1 AND s.expira_em > datetime('now') AND u.admin = 1
        ''', (token,)).fetchone()
        conn.close()
...
def init_db():
    ...
    try:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS sessoes (
            ...
            token TEXT UNIQUE NOT NULL, ip_origem TEXT, expira_em TEXT NOT NULL,
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS tentativas_login (
            id INTEGER PRIMARY KEY AUTOINCREMENT, usuario TEXT, ip_origem TEXT,
            sucesso INTEGER DEFAULT 0, data_tentativa TEXT DEFAULT CURRENT_TIMESTAMP)''')
...
def status_sistema():
    ...
    try:
        ...
        cursor = conn.execute('SELECT COUNT(*) AS total FROM ordens_servico')
        total_os = cursor.fetchone()['total']
        
        cursor = conn.execute("SELECT COUNT(*) AS abertas FROM ordens_servico WHERE upper(status_os) = 'ABERTA'")
        os_abertas = cursor.fetchone()['abertas']
        
        cursor = conn.execute("SELECT COUNT(*) AS fechadas FROM ordens_servico WHERE upper(status_os) = 'FECHADA'")
        os_fechadas = cursor.fetchone()['fechadas']
        
        cursor = conn.execute("SELECT MAX(data_sincronizacao) AS ultima FROM ordens_servico WHERE data_sincronizacao LIKE '____-__-__%'")
        row_u = cursor.fetchone()
        if row_u and row_u['ultima']:
            raw_ultima = row_u['ultima']
        else:
            cursor = conn.execute('SELECT MAX(data_sincronizacao) AS ultima FROM ordens_servico')
            row_u = cursor.fetchone()
            raw_ultima = row_u['ultima'] if row_u and row_u['ultima'] else None
        
        if raw_ultima and 'T' in str(raw_ultima):
            try:
                dt = datetime.fromisoformat(str(raw_ultima).replace('Z', '+00:00'))
                ultima = dt.strftime('%d/%m/%Y %H:%M:%S')
            except:
                ultima = str(raw_ultima)
        else:
            ultima = str(raw_ultima) if raw_ultima else 'Nunca'
        
        cursor = conn.execute("SELECT COUNT(DISTINCT codigo_equip) AS equip_os FROM ordens_servico WHERE upper(status_os) = 'ABERTA'")
        equip_os = cursor.fetchone()['equip_os']
        
        tables = tabelas_permitidas()
        total_equip = conn.execute('SELECT COUNT(*) AS total_eq FROM equipamentos').fetchone()['total_eq'] if 'equipamentos' in tables else 64
        ...
        return jsonify({
            ...
            'data': {
                ...
                'equipOs': equip_os,
                'equipOk': max(0, total_equip - equip_os),
                'ultimaSincronizacao': ultima,
                'tabelasBanco': tables,
                'syncRunning': sync_service.running
            }
        })
    except Exception as exc:
...
<tr><td style="padding:5px;color:#94a3b8;">Frequencia</td><td align="right">{cpu_freq.current:.0f} MHz</td></tr>
        ...
        # Medição Real do Tablet de 2GB RAM via ADB
        mem_total = 1.87
        mem_usada = 1.80
        mem_livre = 0.07
        mem_percent = 96.0
...
def system_info():
    ...
    try:
        # CPU e Memória - Medição REAL do Tablet de 2GB RAM via ADB
        cpu_percent = _cpu_percent
        memoria_total = 1.87
        memoria_usada = 1.80
        memoria_percent = 96.0

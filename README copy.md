...
## Configuracao via .env (novo)

As credenciais do SimpleFarm agora podem (e devem) vir de variaveis de ambiente
em vez de ficarem fixas no codigo:

1. Copie `.env.example` para `.env` na raiz do projeto.
2. Preencha `SF_USERNAME`, `SF_PASSWORD` e demais valores.
3. Instale `python-dotenv` (`pip install python-dotenv`, ja incluso no `requirements.txt`).
4. O backend carrega o `.env` automaticamente ao iniciar. Se o arquivo nao existir,
   o sistema continua funcionando com os valores antigos (fallback), entao nada quebra
   para quem ainda nao migrou.
5. **Nunca** commite o `.env` — ele ja esta no `.gitignore`.

## Saude do Scraper (novo)

Endpoint novo `GET /api/sync/health` retorna o status detalhado da sincronizacao:
falhas consecutivas, ultimo erro, ultimo sucesso e historico recente. Os mesmos
campos tambem foram adicionados (sem remover nenhum campo antigo) dentro de
`sync` em `/api/system`.
...
O scraper agora tambem:
- Tenta o login ate 3x com backoff curto antes de desistir do ciclo.
- Usa backoff adaptativo (30s -> ate 5min) quando ha falhas consecutivas,
  para nao martelar o servidor do SimpleFarm durante uma indisponibilidade.
- Registra o erro especifico (timeout, conexao, status inesperado) em vez de
  simplesmente "falhou".
